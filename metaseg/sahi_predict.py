...
from metaseg.utils import download_model, load_image, multi_boxes, plt_load_box, plt_load_mask,load_image_vide
...
class SahiAutoSegmentation:
    ...
    def predict(
        ...
        read_vide = load_image_vide(source)
        model = self.load_model(model_type)
        predictor = SamPredictor(model)
        predictor.set_image(read_vide)

        if type(input_box[0]) == list:
            input_boxes, new_boxes = multi_boxes(input_box, predictor, read_vide)
